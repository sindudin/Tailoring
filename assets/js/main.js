...
$(document).ready(function(){  
  ...
  $(".add-row").click(function(){
    ...
    if(jobTxt=="" || quantityValue==""){

    }else{
      var AddMarkUp = '<div class="add-row-wapper">\
                      <div class="input-wapper">\
                          <div class="col-sm-7 col-sm-offset-2">\
                              <input type="text" class="form-control" id="input01" value='+ jobTxt +'>\
                          </div>\
                      </div>\
                      <div class="input-wapper">\
                          <div class="col-sm-1 col-sm-offset-1">\
                              <input type="text" class="form-control" id="input01" value='+ quantityValue +'>\
                          </div>\
                          <div class="col-sm-1">\
                              <span class="input-group-btn"><button class="delete-row btn bootstrap-touchspin-up btn-lightred icon-trash" title="Remove job" type="button"></button></span>\
                          </div>\
                      </div>\
                  </div>';
    }
    ...
    if(window.rowCount >= 1){
      $('.add-row-wapper:first-child').after(AddMarkUp);
    }else{
      $('.add-row-wapper:last-child').after(AddMarkUp);
    }
    
    $(this).parents('.add-row-wapper').find('#jobTxt').val('');
  ...
  $("body").on('click','.delete-row',function(ev){
    $(this).parents('.add-row-wapper').remove();
  });

  $('.t-tool-txt').hover(function(ev){
    var left = ev.offsetX;
    var top = ev.offsetY;
    var txtWidth = $(this).outerWidth();
    $(this).next('.t-tooltip-wapper').css('left',txtWidth);
    //$(this).next('.t-tooltip-wapper').css('display','block');
  });
});
